...
{
    private readonly PooledArrayBufferWriter<byte>? _writer;

    /// <summary>Initializes an <see cref="OwnedBuffer"/> wrapping the given writer.</summary>
    internal OwnedBuffer(PooledArrayBufferWriter<byte> writer) => _writer = writer;

    /// <summary>Returns the span of bytes written to the underlying writer.</summary>
    internal ReadOnlyMemory<byte> Memory => _writer?.WrittenMemory ?? default;
