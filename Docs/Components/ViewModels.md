...
**Properties**: `IsInstallScreenVisible`, `IsKioskMode`, `UserName`, `EnvironmentTitle`, `EnvironmentColor`, `AppVersion`, `IsServerMode`/`IsClientMode` (fixed for the process's lifetime, from `IEngineController.Role`), `ShowMainLayout` (`!IsServerMode && !IsInstallScreenVisible`), `ShowConnectionsTable` (`IsServerMode && !IsInstallScreenVisible && !IsServerActivityViewActive`), `ShowServerActivityView` (`IsServerMode && !IsInstallScreenVisible && IsServerActivityViewActive`), plus `FolderBar (IFolderBarViewModel)`, `EntryBar (IEntryBarViewModel)`, `ContentArea (IContentAreaViewModel)`, `InstallView (IInstallViewModel)`, `Alert (IAlertViewModel)`, `Export (IExportViewModel)`, `Import (IImportViewModel)`, `PrintManager (IPrintManagerViewModel)`, `ConnectionStatus (IConnectionStatusViewModel)`.
