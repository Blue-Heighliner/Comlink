...
A `NodeRole.Client` instance keeps the normal layout, with a single `ConnectionRow` (bound to `ConnectionStatus.ServerRows[0]`, since a client's own connection is to a server) pinned to the bottom of the window, visible only while `IsClientMode` is `true`.
...
Actually playing the alarm sound is driven through a separate `IAlertSoundPlayer` constructor dependency, not `IEngineController` — see `Docs/Components/Control.md`'s `IAlertSoundPlayer` entry (not a control interface; always real Engine behavior, like printer discovery/driving).
...
- The tag section of Sending a message appears only when `TagsEnabled`, and the alert section only when `ComposeAlertsEnabled`; both, and the alert section of Receiving messages, are titled with the host's own `TagLabel`/`AlertLabel`.
...
`Peer.IConnectionStatusService` is registered per `NodeRole` (see `Docs/Components/Peer.md#node-roles`): for
