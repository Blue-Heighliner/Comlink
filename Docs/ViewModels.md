...
`AlertLabel` is set once from `IAlertConfiguration.AlertText` — the same interface and value that drives the title bar's alert box (`AlertViewModel.AlertText`), so the alert checkbox in `DraftEditor.axaml` and the title bar's alarm box always agree on what "alert" is called (e.g. Sample's `"!ALERT!"` override applies to both). `ComposeAlertsEnabled` is set once from `IAlertComposeConfiguration.ComposeAlertsEnabled`; `DraftEditor.axaml` hides the alert checkbox entirely when `false` (combined with `!IsSent` via a `MultiBinding`), so a draft can never be marked or sent as an alert from this app while disabled — this has no effect on receiving or alarming on a peer-originated alert. See `Docs/Control.md`.

`AvailablePriorities` starts from `IMessagePriorityProvider.GetPriorities()` (see `Docs/Control.md`) but is **not** static: it is recomputed every time `Tag` changes, excluding any priority that `IMessageTagPriorityPolicy` blocks for the current tag, so a blocked combination is never even offered in the priority `ComboBox`. `SelectedPriority` initializes to the option whose `Value` matches `DraftEntity.Priority` within the tag-filtered list, falling back to the first available option (and finally a hard-coded `Value = 0` option) if the stored value no longer matches any currently available option — e.g. after a host changes its `IMessagePriorityProvider`/`IMessageTagPriorityPolicy` registration, or because the stored (tag, priority) pair is itself blocked. `SelectedPriority.Value` is persisted on `DraftEntity.Priority` on save, and passed through `IServiceConnection.SendMessage(..., priority)` on send, from which it flows to the OFT send priority — see [Peer.md](Peer.md).

`Tag` is persisted on `DraftEntity.Tag` across save/reload and passed through `IServiceConnection.SendMessage(..., tag)` on send. Setting `Tag` to a value that `IMessageTagPriorityPolicy` blocks in combination with the currently-selected `SelectedPriority` is rejected outright — the value silently reverts to the last accepted tag instead of taking effect — so a blocked tag/priority combination can never actually be entered in the draft editor; `SendCommand` also re-checks before sending as a defense-in-depth safety net (setting `StatusMessage` and refusing to send if it is somehow still blocked). `TagsEnabled` is set once from `IMessageTagConfiguration.TagsEnabled`; `DraftEditor.axaml` hides the tag `TextBox` entirely when `false` (combined with `!IsSent` via a `MultiBinding`), and `EntryBarViewModel` likewise omits `TagText` from the entry listing. `TagLabel` is set once from `IMessageTagConfiguration.TagLabel` and drives the tag `TextBox`'s watermark, letting a host rename the concept (e.g. Sample's `"Category"`) without changing engine behavior — see `IMessageTagConfiguration` in `Docs/Control.md`.
