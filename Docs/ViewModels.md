...
Actually playing the alarm sound is driven through a separate `IAlertSoundPlayer` constructor dependency, not `IEngineController` — see `Docs/Control.md`'s `IAlertSoundPlayer` entry (not a control interface; always real Engine behavior, like printer discovery/driving).
