...
// server. Run via Run.task, or standalone (after Build.cs) in its own terminal alongside Server2.cs,
// Client1.cs, and Client2.cs, from the repo root:
//   dotnet run Scripts/Scenarios/ServerCluster/Server1.cs

using Markwardt.ScriptUtilities;

(await Script.Run("dotnet", "run", "--no-build", "--project", "Sample/Sample.csproj", "--", "--config", "Scripts/Scenarios/ServerCluster/Server1.json")).Verify();
