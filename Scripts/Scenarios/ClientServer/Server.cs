...
// its own terminal alongside Client1.cs and Client2.cs, from the repo root:
//   dotnet run Scripts/Scenarios/ClientServer/Server.cs

using Markwardt.ScriptUtilities;

(await Script.Run("dotnet", "run", "--no-build", "--project", "Sample/Sample.csproj", "--", "--config", "Scripts/Scenarios/ClientServer/Server.json")).Verify();
